...
Josh Gilliam
Credits: Copilot helped me with closest_index and assign_closest functions.
"""
...
def closest_index(point: tuple[int, int], centroids: list[tuple[int, int]]) -> int:
    ...
    """
    closest_i = 0
    smallest_dist = sq_dist(point, centroids[0])
    for i in range(1, len(centroids)):
        d = sq_dist(point, centroids[i])
        if d < smallest_dist:
            smallest_dist = d
            closest_i = i
    return closest_i
...
def assign_closest(points: list[tuple[int,int]],
    ...
    for i in range(len(centroids)):
        assignments.append([])
    for point in points:
        i = closest_index(point, centroids)
